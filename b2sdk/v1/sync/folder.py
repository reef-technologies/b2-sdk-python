...
def translate_errors(func):
    ...
    def wrapper(*a, **kw):
        try:
            ...
            raise exception.CommandError('Directory %s is empty.  Use --allowEmptySource to sync anyway.' % (ex.path,))
...
class B2Folder(v2.B2Folder, AbstractFolder):
    ...
    def get_file_versions(self):
        for file_version, _ in self.bucket.ls(self.folder_name, show_versions=True, recursive=True):
            yield file_version
